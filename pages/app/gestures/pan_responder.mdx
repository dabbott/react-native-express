...
> In a real app, we should use `Animated.Value`s in our style for better performance. E.g. create a `positionY` animated value from `initialY` and `offsetY`, and update whenever the state of the drag changes.
