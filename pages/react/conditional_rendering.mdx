...
The TypeScript type `React.ReactNode` includes all of these.
...
> Note that a buttonTitle equal to the empty string, `""`, won't render. If we did want it to, we could use `buttonTitle === 'undefined'` to check for that case.
