...
- Use [`react-navigation`](/app/navigation/react_navigation) to add a "details" screen that appears when tapping a photo - the `list` API which we're using already returns some photo metadata that you could display.
